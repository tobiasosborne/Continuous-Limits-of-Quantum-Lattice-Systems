...
Given such a hilbert space of fluctuations we now implement a version of the \emph{Wightman reconstruction procedure} to obtain a representation for the hamiltonian (actually, the unitary translation operator) describing the low-energy large-scale dynamics of the tight-binding model. Our discussion will be necessarily heuristic as we haven't yet introduced the mathematical formalism to make it rigourous (we do this in a later appendix). 

The idea behind our approach is as follows. Recall that our hilbert space of fluctuations is spanned by vectors of the form
\begin{equation}
	|f_1, f_2, \ldots, f_m\rangle \equiv \widehat{\psi}^\dag(f_1)\widehat{\psi}^\dag(f_2)\cdots \widehat{\psi}^\dag(f_m)|\Omega\rangle.
\end{equation}
The way we obtain the hamiltonian (and momentum) operator is to deduce a representation of a unitary operation $U(x_0,t_0)$ implementing the transformation $\widehat{\psi}(f(x,t)) \mapsto \widehat{\psi}(f(x-x_0,t-t_0))$. Differentiating this unitary with respect to $x_0$ and $t_0$ then furnishes a representation of the momentum and hamiltonian operators, respectively. The main problem is then to show that the translation operation $(x,t)\mapsto (x+x_0, t+t_0)$ is: (a) well defined; (b) linear; and (c) unitary on the hilbert space of fluctuations.

To understand whether the translation operation is well defined we first define it for all vectors in $\widehat{\mathcal{H}}$ by extending the map 
\begin{equation}
	\mathcal{T}(|f_1, f_2, \ldots, f_m\rangle) \equiv |f_1', f_2', \ldots, f_m'\rangle,
\end{equation}
where
\begin{equation}
	f_j'(x,t) \equiv f(x-x_0,t-t_0), \quad j = 1, 2, \ldots, m,
\end{equation}
by linearity. This definition makes it clear that, in the very least, $\mathcal{T}$ is a \emph{relation} on $\widehat{\mathcal{H}}\times \widehat{\mathcal{H}}$, i.e., $\mathcal{T}\subseteq \widehat{\mathcal{H}}\times \widehat{\mathcal{H}}$ We also see that $\widehat{\mathcal{H}}$ is the domain of $\mathcal{T}$. Thus we are left with the task of showing that if $(|\phi\rangle, |\psi\rangle)$ and $(|\phi\rangle, |\psi'\rangle)$ are elements of the relation defined by $\mathcal{T}$ then $|\psi\rangle = |\psi'\rangle$. The only difficulty arises when $|\phi\rangle$ admits two different representations as linear combinations of the generating vectors $|f_1, f_2, \ldots, f_m\rangle$. Suppose that
\begin{equation}
	|\phi\rangle = \sum_{j} c_{j} |f_1^{(j)}, f_2^{(j)}, \ldots, f_{m_j}^{(j)}\rangle
\end{equation}
and
\begin{equation}
	|\phi\rangle = \sum_{j} d_{j} |g_1^{(j)}, g_2^{(j)}, \ldots, g_{m_j}^{(j)}\rangle
\end{equation}
are two different representations of $|\phi\rangle$ as a linear combination of the generating vectors. Our claim is then that 
\begin{equation}
	\sum_{j} c_{j} \mathcal{T}(|f_1^{(j)}, f_2^{(j)}, \ldots, f_{m_j}^{(j)}\rangle) = \sum_{j} d_{j} \mathcal{T}(|g_1^{(j)}, g_2^{(j)}, \ldots, g_{m_j}^{(j)}\rangle).
\end{equation}
To argue this we first suppose the sum is finite and induct on the maximum number $\max_{j} m_j$ of functions appearing inside the ket. We look at the base case: exploit the multilinearity of the ket, i.e.,
\begin{equation}
	|f_1, \ldots, f_k + g_k, \ldots f_m\rangle = |f_1, \ldots, f_k, \ldots f_m\rangle + |f_1, \ldots, g_k, \ldots f_m\rangle,
\end{equation}
which directly follows from the definition of the quantum continuous limit. Consider $\mathcal{T}$ separately applied to both sides:
\begin{equation}
	\mathcal{T}(|f_1, \ldots, f_k + g_k, \ldots f_m\rangle) = |f_1', \ldots, f_k' + g_k', \ldots f_m'\rangle
\end{equation}
and
\begin{equation} 
	\mathcal{T}(|f_1, \ldots, f_k, \ldots f_m\rangle) + \mathcal{T}(|f_1, \ldots, g_k, \ldots f_m\rangle) = |f_1', \ldots, f_k', \ldots f_m'\rangle + |f_1', \ldots, g_k', \ldots f_m'\rangle.
\end{equation}
Exploiting the multilinearity of the ket with respect to its argument we see that both representations of the image are identical. This argument is easily extended, via completion, to the general case. So at least we know that $\mathcal{T}$ is a linear map. 

The only remaining issue now is to argue that $\mathcal{T}$ is \emph{unitary}. That is, applying $\mathcal{T}$ does not destroy (or create) probability. To this end we study the behaviour of the inner product 
\begin{equation}
	\langle f_1, \ldots, f_k, \ldots f_m|g_1, \ldots, g_k, \ldots g_m\rangle
\end{equation}
under application of $\mathcal{T}$. We start with the one-particle sector:
\begin{equation}
	\langle f|g\rangle \equiv  \lim_{a\rightarrow 0} a^3 \sum_{j,j'\in\mathbb{Z}} \int_{-\infty}^\infty \int_{-\infty}^\infty e^{2\pi i (-jk_0+j'k_0')}f(at, aj)g(at', aj') \langle \Omega|c_j e^{i(t-t')H_a}c_{j'}^\dag e^{-i(t-t')}|\Omega\rangle \, dt.
\end{equation}
It is immediately clear that this expression only depends on the \emph{difference} $t-t'$, so that a translation in time leads to no change in the inner product. Similarly, translation invariance of the ground state $|\Omega\rangle$ immediately leads to the same conclusion for spatial translations. The general case follows from similar manipulations. Thus we deduce the action of $\mathcal{T}$ is unitary on the space of fluctuations.
...
\subsection{An alternative construction for general local field operators via blocking}

Here we explore an alternative way to build general local field operators based on \emph{blocking}. The idea is very simple: instead of considering translates of local operators in $\mathcal{A}(\Lambda_n)$ we first block our lattice into blocks $B_n(j)$ of side $n$ with basepoint $j\in n\mathbb{Z}^D$, i.e.\
\begin{equation}
	B_n(j) \equiv j+ \{0,1,\ldots, n-1\}^{\times D}. 
\end{equation} 
We then form the following classical field operators
\begin{equation}
	\phi^{(a)}_n(f) = (na)^D \sum_{j\in n\mathbb{Z}^D} \sum_\alpha [f]_\alpha (aj) \lambda_j^\alpha 
\end{equation}
and $\lambda_j^\alpha$ is a basis for $\mathcal{A}(B_n(j))$. The fluctuation fields are now given by
\begin{equation}
	\widetilde{\phi}^{(a)}_n(f) = Z_f(a)\left((na)^D \sum_{j\in n\mathbb{Z}^D} \sum_\alpha [f]_\alpha (aj) \lambda_j^\alpha - \langle \phi_n(f)\rangle \mathbb{I}\right).
\end{equation}
This construction simply reduces to the original continuum limit procedure instead applied to lattice supersites given by blocks of lattice points of side $n$. In this case the symplectic matrix $\Omega$ determining the canonical fields is directly determined by the reduced density operator of the block.

A crucial question is now whether the construction presented here is equivalent to the general field operator construction of the previous section. It turns out that this is not always the case and we now spell out the conditions according to which the two constructions are equivalent. 
