...
The simplest nontrivial of local fermion models is the spinless tight binding model in one dimension
\begin{equation}
	H = \sum_{j\in \mathbb{Z}} a_{j+1}^\dag a_j + \text{h.c.}
\end{equation}
...
\begin{equation}
	\begin{split}
		\widehat{a}(k) &\equiv \sum_{j\in\mathbb{Z}} e^{2\pi i jk} a_j, \quad \text{and} \\
		\widehat{a}^\dag(k) &\equiv \sum_{j\in\mathbb{Z}} e^{-2\pi i jk} a_j^\dag,
	\end{split}
\end{equation}
which obey the canonical anticommutation relations
\begin{equation}
	\{\widehat{a}(k), \widehat{a}^\dag(k')\} = \sum_{j\in\mathbb{Z}} e^{2\pi i j(k-k')}\mathbb{I} = \delta(k-k')\mathbb{I}.
\end{equation}
Inverting this relationship we find 
\begin{equation}
	a_j = \int_{-\frac12}^{\frac12} dk\, e^{-2\pi ijk} \widehat{a}(k).
\end{equation}
In terms of the fourier variables the hamiltonian is diagonal:
\begin{equation}
	\begin{split}
		H &= \sum_{j\in \mathbb{Z}} a_{j+1}^\dag a_j + \text{h.c.} \\
		&= \sum_{j\in \mathbb{Z}}\int_{-\frac12}^\frac12 \int_{-\frac12}^\frac12 dkdk'\, e^{2\pi i (j+1)k- 2\pi i j k'} \widehat{a}^\dag(k) \widehat{a}(k') + \text{h.c.} \\ 
		&= \int_{-\frac12}^\frac12 \int_{-\frac12}^\frac12 dkdk'\, \delta(k-k') e^{2\pi i} \widehat{a}^\dag(k) \widehat{a}(k') + \text{h.c.} \\ 
		&= 2\int_{-\frac12}^\frac12 dk\, \cos(2\pi k) \widehat{a}^\dag(k) \widehat{a}(k).
	\end{split}
\end{equation}
The ground state $|\Omega\rangle$ is then determined by the condition that 
\begin{equation}
\langle \Omega|\widehat{a}^\dag(k) \widehat{a}(k)|\Omega\rangle = \begin{cases}
	1, &\quad k \not\in (-\frac14, \frac14), \\
	0, &\quad k \in (-\frac14, \frac14).
\end{cases}
\end{equation}
...
\begin{equation}
	\begin{split}
		\langle \lambda_j^3\rangle &= \frac{1}{\sqrt2}\int_{-\frac12}^{\frac12}\int_{-\frac12}^{\frac12} dkdk'\, e^{2\pi ij (k-k')} \langle \widehat{a}^\dag(k)\widehat{a}(k')\rangle - \frac{1}{\sqrt2}\int_{-\frac12}^{\frac12}\int_{-\frac12}^{\frac12} dkdk'\, e^{2\pi ij (k'-k)} \langle \widehat{a}(k)\widehat{a}^\dag(k')\rangle \\
		&= \frac{1}{\sqrt2}\int_{-\frac12}^{\frac12} dk\,  \langle \widehat{a}^\dag(k)\widehat{a}(k)\rangle - \frac{1}{\sqrt2}\int_{-\frac12}^{\frac12} dk\, \langle \widehat{a}(k)\widehat{a}^\dag(k)\rangle = 0.
	\end{split}
\end{equation}
The remaining expectation values follow a similar pattern; consider the classical two-point function
\begin{equation}
	\begin{split}
		\langle a_0^\dag a_l\rangle &= \int_{-\frac12}^{\frac12}\int_{-\frac12}^{\frac12} dkdk'\, e^{-2\pi ilk'} \langle \widehat{a}^\dag(k)\widehat{a}(k')\rangle \\
		&= \int_{-\frac12}^{\frac12} dk\, e^{-2\pi ilk} \langle \widehat{a}^\dag(k)\widehat{a}(k)\rangle \\
		&= \frac{e^{-2\pi ilk}}{-2\pi i l}\bigg|_{k=-\frac12}^{k=\frac12} - \int_{-\frac14}^{\frac14} dk\, e^{-2\pi ilk}\\ 
		&= -\frac{\cos(\pi i l)}{\pi i l} - \frac{e^{-2\pi ilk}}{-2\pi i l}\bigg|_{k=-\frac14}^{k=\frac14} \\
		&= \frac{\cos(\frac{\pi i}{2}l)-\cos(\pi il)}{\pi i l} = \frac{\cos(\frac{\pi i}{2}l)-(-1)^l}{\pi i l}.
	\end{split}
\end{equation}
That is, the classical correlation function is \emph{nontrivial}. 

The tight-binding model exposes a feature that we have hitherto neglected, namely, that we can build different \emph{inequivalent} continuum limits around \emph{different} lattice momenta. What this means in the present context is as follows. Write
\begin{equation}
	\phi^{(a)}_k(f) \equiv a\sum_{j\in\mathbb{Z}} \sum_{\alpha=0}^{2^s-1} e^{2\pi i jk}[f]_\alpha(aj) \lambda_j^\alpha,
\end{equation}
where $k \in [-\frac12, \frac12)$. This is intended to be a classical observable based around lattice momentum $k$: this is engineered via the addition of the fourier-like phase factor. Correspondingly, we also the introduce the quantum fluctuation operator around lattice momentum $k$:
\begin{equation}
...
One sees that, apart from special cases, the fluctuation operators for different values of $k$ will generically (anti)commute with each other, i.e., the fluctuations decouple. (This is a straightforward application of the Riemann-Lebesgue lemma.) 

In the case of the tight-binding model we encounter nontrivial features due to the fermi surface at $k = \pm \frac14$.
...
So far our discussion has focussed on the observable algebra given by the quasi-local algebra $\mathcal{A}(\mathbb{Z}^D)$. This is generically the most appropriate collection of observables for a local quantum system: suppose that $|\Omega\rangle$ is the ground state of some local quantum system and let $A$ be some \emph{extensive} string-like observable, i.e., $A = \bigotimes_{j} a_j$, $a_j\in \mathcal{A}$. Then we generically expect $A|\Omega\rangle$ to lie in another hilbert space. Ordinarily, on energy grounds, we are free to ignore the hilbert space built from $A|\Omega\rangle$ as irrelevant to low-energy physics. However, there are situations where we must include this hilbert space in the discussion of low-energy large-scale physics, namely, when the system is at a symmetry breaking transition. 

A complete mathematical treatment of this problem would bring us to the discussion of superselection sectors. We'll evade this somewhat by specialising to (a) one spatial dimension; and (b) the case of systems invariant under local symmetry actions. Let's begin with a fairly general class of examples. Suppose $|\Omega_a\rangle$, $a>0$, is a sequence of states of a one-dimensional chain of qudits of dimension $d$. We further assume that this sequence of states admits a local action of a symmetry group $G$. What this means is that $G$ acts on $|\Omega_a\rangle$ via the representation
\begin{equation}
