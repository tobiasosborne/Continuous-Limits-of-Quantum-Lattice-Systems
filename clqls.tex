...
\begin{equation}
	\begin{split}
		\langle {\widetilde{\psi}^\dag}{}^{(a)}_{-\frac{1}{4}}(f)\widetilde{\psi}^{(a)}_{\frac{1}{4}}(g) \rangle &= a\sum_{j,j'}f(aj)g(aj')\int_{-\frac12}^{\frac12}\int_{-\frac12}^{\frac12} dkdk'\, e^{2\pi i (jk-j'k')}e^{\frac{\pi}{2} i (j+j')} \langle \widehat{c}^\dag(k)\widehat{c}(k')\rangle \\
		&= \frac{1}{a}\int_{-\frac12}^{\frac12}\int_{-\frac12}^{\frac12} dkdk'\, \widehat{f}(\widetilde{k})\widehat{g}(\widetilde{k}') \langle \widehat{c}^\dag(k)\widehat{c}(k')\rangle + O(a),\\
		&= \frac{1}{a}\int_{-\frac14}^{\frac14} dk\, \widehat{f}(\widetilde{k})\widehat{g}(\widetilde{k}) + O(a),
	\end{split}
\end{equation}
where 
\begin{equation}
...
\end{equation}
and
$\widetilde{k} \equiv \frac{1}{a}(k+\frac14)$. Thus the correlation function is determined by 
\begin{equation}
	\frac{1}{a}\int_{-\frac14}^\frac14 \, \widehat{f}(\widetilde{k})\widehat{g}(\widetilde{k})\, dk \rightarrow \int_0^{\infty} \widehat{f}(k)\widehat{g}(k) \, dk,
\end{equation}
where $\omega(k) = k+\frac14$, $\Lambda = 1/a$.
