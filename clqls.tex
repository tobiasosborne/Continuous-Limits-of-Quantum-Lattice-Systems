...
\end{equation}
by linearity. This definition makes it clear that, in the very least, $\mathcal{T}$ is a \emph{relation} on $\widehat{\mathcal{H}}\times \widehat{\mathcal{H}}$. We also see that $\widehat{\mathcal{H}}$ is the domain of $\mathcal{T}$. Thus we are left with the task of showing that if $(|\phi\rangle, |\psi\rangle)$ and $(|\phi\rangle, |\psi'\rangle)$ are elements of the relation defined by $\mathcal{T}$ then $|\psi\rangle = |\psi'\rangle$.
