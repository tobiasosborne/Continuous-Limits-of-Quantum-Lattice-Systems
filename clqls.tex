...
The simplest nontrivial model of local fermion models is the spinless tight-binding model in one dimension:
\begin{equation}
	H = \sum_{j\in \mathbb{Z}} c_{j+1}^\dag c_j + \text{h.c.}
\end{equation}
...
\begin{equation}
	\begin{split}
		H &= \sum_{j\in \mathbb{Z}} c_{j+1}^\dag c_j + \text{h.c.} \\
		&= \sum_{j\in \mathbb{Z}}\int_{-\frac12}^\frac12 \int_{-\frac12}^\frac12 dkdk'\, e^{2\pi i (j+1)k- 2\pi i j k'} \widehat{c}^\dag(k) \widehat{c}(k') + \text{h.c.} \\ 
		&= \int_{-\frac12}^\frac12 \int_{-\frac12}^\frac12 dkdk'\, \delta(k-k') e^{2\pi i k} \widehat{c}^\dag(k) \widehat{c}(k') + \text{h.c.} \\ 
		&= 2\int_{-\frac12}^\frac12 dk\, \cos(2\pi k) \widehat{c}^\dag(k) \widehat{c}(k).
	\end{split}
\end{equation}
The ground state $|\Omega\rangle$ is then determined by the condition that 
\begin{equation}
\langle \Omega|\widehat{c}^\dag(k) \widehat{c}(k)|\Omega\rangle = \begin{cases}
	1, &\quad k \not\in (-\frac14, \frac14), \\
...
\begin{equation}
	...
	\end{split}
\end{equation}
The correlation function averages to zero according to the standard continuum limit built around lattice momentum $k=0$. However, the classical correlation function will be \emph{nontrivial} for a continuum limit built around lattice momenta $k=\frac14$. Thus the tight-binding model exposes a feature that we have hitherto neglected, namely, that we can build different \emph{inequivalent} continuum limits around \emph{different} lattice momenta. This becomes a necessity when the low-energy physics is determined by a minimum in the dispersion relation occurring at nonzero lattice momenta --- a generic situation for fermion models. What this means in the present context is as follows. Write
\begin{equation}
	\phi^{(a)}_k(f) \equiv a\sum_{j\in\mathbb{Z}} \sum_{\alpha=0}^{2^s-1} e^{2\pi i jk}[f]_\alpha(aj) \lambda_j^\alpha,
\end{equation}
where $k \in [-\frac12, \frac12)$. This is intended to be a classical observable based around lattice momentum $k$: this is engineered via the addition of the fourier-like phase factor. 

When we choose $k = \frac14$ we see that the classical fields 
\begin{equation}
	\psi^{(a)}_{\frac{1}{4}}(f) \equiv a\sum_{j\in\mathbb{Z}} e^{\frac{\pi}{2} i j}f(aj) c_j, \quad \text{and} \quad {\psi^\dag}^{(a)}_{-\frac{1}{4}}(f) \equiv a\sum_{j\in\mathbb{Z}} e^{\frac{\pi}{2} i j}f(aj) c_j^\dag
\end{equation}
exhibit nontrivial correlations in the classical limit:
\begin{equation}
	\begin{split}
		\langle {\psi^\dag}^{(a)}_{-\frac{1}{4}}(f)\psi^{(a)}_{\frac{1}{4}}(g) \rangle &= a^2\sum_{j,j'}f(aj)g(aj')\int_{-\frac12}^{\frac12}\int_{-\frac12}^{\frac12} dkdk'\, e^{2\pi i (jk-j'k')}e^{\frac{\pi}{2} i (j+j')} \langle \widehat{c}^\dag(k)\widehat{c}(k')\rangle \\
		&= \int_{-\frac12}^{\frac12}\int_{-\frac12}^{\frac12} dkdk'\, \widehat{f}(\widetilde{k})\widehat{g}(\widetilde{k}') \langle \widehat{c}^\dag(k)\widehat{c}(k')\rangle,\\
		&= \int_{-\frac12}^{\frac12} dk\, \widehat{f}(\widetilde{k})\widehat{g}(\widetilde{k}) - \int_{-\frac14}^{\frac14} dk\, \widehat{f}(\widetilde{k})\widehat{g}(\widetilde{k}).
	\end{split}
\end{equation}
where $\widetilde{k} \equiv \frac{1}{a}(k+\frac14)$.


We also the introduce the quantum fluctuation operator around lattice momentum $k$:
\begin{equation}
